...
KCLManager.prototype._onRecordProcessorAction = function(action) {
  ...
  if (actionType === 'initialize') {
    ...
    recordProcessorFuncInput.checkpointer = checkpointer;
    recordProcessorFunc = recordProcessor.shutdownRequested;
    beginActionInput = 'beginShutdownRequested';
    finishActionInput = 'finishShutdownRequested';
  }
  // Should not occur.
  else {
